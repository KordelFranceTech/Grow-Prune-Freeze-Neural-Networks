"""
Expected SARSA (neural network) training script for plume navigation.

Usage:
    python3 train.py
    python3 train.py --config configs/default.yaml --seed 7
    python3 train.py --config configs/default.yaml --no-viz
"""
...
def train(cfg, seed=None, visualize=True):
    ...
    save_dir = rl_cfg.get('output_dir', 'checkpoints/rl/')
    os.makedirs(save_dir, exist_ok=True)

    input_dim = sum(env.state_shape)
    print(f"Training Expected SARSA (NN)  —  {n_episodes} episodes")
    ...
    if visualize:
        ...
        # Load best weights if we saved any, otherwise use current
        best_path = os.path.join(save_dir, 'network_best.pt')
